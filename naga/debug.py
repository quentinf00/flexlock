...
def unsafe_debug(fn):
    ...
    """
    naga_debug = os.environ.get('NAGA_DEBUG', 'false').lower() in ('1', 'true')
    ...
    def _fn(*args, **kwargs):
        try:
            ...
            print(f"--- NAGA_DEBUG: An exception occurred in {fn.__name__}: {exc_value} ---", file=sys.stderr)
            print(f"--- NAGA_DEBUG: Locals in '{fn.__name__}' at time of error: {fn_locals} ---", file=sys.stderr)
            ...
            try:
                ...
                print(f"--- NAGA_DEBUG: Injected locals into '{caller_frame.f_code.co_name}'. ---", file=sys.stderr)
            finally:
