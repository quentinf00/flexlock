...
def test_debug_decorator_inactive_by_default():
    ...
    assert "x" not in locals(), (
        "Variable 'x' should not be injected when decorator is inactive"
    )
    assert "y" not in locals(), (
        "Variable 'y' should not be injected when decorator is inactive"
    )
...
def test_debug_decorator_explicitly_inactive():
    ...
    assert "x" not in locals(), (
        "Variable 'x' should not be injected when FLEXLOCK_DEBUG='0'"
    )
    assert "y" not in locals(), (
        "Variable 'y' should not be injected when FLEXLOCK_DEBUG='0'"
    )
...
def test_debug_decorator_active_and_injects_on_exception():
    ...
    assert "a" in locals(), (
        "Variable 'a' should have been injected by the debug decorator"
    )
    assert "b" in locals(), (
        "Variable 'b' should have been injected by the debug decorator"
    )
...
def test_debug_decorator_active_with_true_string():
    ...
    assert "text_var" in locals(), (
        "Variable should be injected when FLEXLOCK_DEBUG='true'"
    )
    assert locals()["text_var"] == "hello", (
        "Injected variable should have the correct value"
    )
