...
def test_mlflowlink_starts_run_and_sets_tags(dummy_run_files, mock_mlflow_module):
    ...
    mock_mlflow_module.set_tag.assert_any_call(
        "flexlock.logical_run_id", str(dummy_run_files.as_posix())
    )
    mock_mlflow_module.set_tag.assert_any_call("flexlock.run_status", "active")
    mock_mlflow_module.set_tag.assert_any_call(
        "flexlock.supersedes_run_id", "previous_run_id"
    )
...
def test_mlflowlink_logs_artifacts_and_params(dummy_run_files, mock_mlflow_module):
    ...
    expected_params = {
        ...
        "config.nested.key": 123,
        "config.save_dir": str(dummy_run_files),
    }
    mock_mlflow_module.log_params.assert_called_once_with(
        expected_params, run_id="test_run_id"
    )
    mock_mlflow_module.log_artifact.assert_any_call(
        str(dummy_run_files / "run.lock"), run_id="test_run_id"
    )
    mock_mlflow_module.log_artifact.assert_any_call(
        str(dummy_run_files / "experiment.log"), run_id="test_run_id"
    )
...
def test_mlflowlink_deprecates_previous_run(dummy_run_files, mock_mlflow_module):
    ...
    with patch.dict(
        "sys.modules", {"mlflow": mock_mlflow_module, "pandas": MagicMock()}
    ):
        with mlflowlink(path=str(dummy_run_files)):
            pass
    ...
    mock_mlflow_module.set_tag.assert_any_call(
        "flexlock.superseded_by_run_id", "test_run_id"
    )
